...
let getModelName = function(id) {
  return id.split(".")[0]
}
let getMethodName = function(id) {
  let i = id.indexOf(".")
  let method = id.substring(i+1).replace('prototype.', '')
  return normalizeName(method)
}
...
let getViewForSwagger = function (opts) {
  ...
  _.forEach(swagger.paths, function (api, path) {
    ...
    _.forEach(api, function (op, m) {
      ...
      let modelName = getModelName(op.operationId)
      let model = _.find(data.models, function(obj) {
        return obj.name == modelName;
      })
      ...
      let method = {
        path: path,
        name: getMethodName(op.operationId),
        className: opts.className,
...
let parse = function (opts) {
  let data = getViewForSwagger(opts)
  return data
}
